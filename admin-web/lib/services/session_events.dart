...
///
/// Транспорт (`ApiClient`) узнаёт об этом первым: обновить токен не
/// удалось, хранилище очищено. До этого события провайдер авторизации о
/// случившемся не знал вовсе и оставался в состоянии «вошёл», а экраны
/// продолжали показывать данные, за которыми уже нельзя сходить, — OSB-M I5.
///
...
/// каждый такой стенд. Здесь подписчик один — `AuthProvider`.
class SessionEvents {
  SessionEvents._();
