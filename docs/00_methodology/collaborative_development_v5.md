...
Это новый уровень в архитектуре v6.0, который решает критическую проблему интеграции сгенерированного кода в реальную структуру проекта. IDE-агенты, такие как Cursor AI, работают непосредственно в среде разработки и выполняют роль исполнительного механизма.
...
1. Откройте Cursor с репозиторием restaurant-guide-belarus
2. Убедитесь что вы на main ветке и сделали pull последних изменений
3. Следуйте инструкциям ниже работая с Cursor AI
...
Третий шаг это интеграция через IDE-агента. Человек-координатор передает инструкции Cursor AI в chat по шагам, IDE-агент создает файлы и вставляет код согласно инструкциям, человек-координатор проводит быструю визуальную проверку созданной структуры, при обнаружении проблем возвращается к операционному Стволу для уточнений, после успешной проверки IDE-агент или человек выполняет commit с предложенным сообщением, выполняется push в GitHub репозиторий.
...
Антипаттерн ignoring IDE-agent suggestions. Cursor AI иногда предлагает улучшения или указывает на проблемы при интеграции кода. Автоматическое принятие без анализа или автоматическое отклонение без рассмотрения, обе крайности проблематичны. Лучше quickly evaluate каждое suggestion на relevance.
